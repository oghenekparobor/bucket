# Railway builds the LAST stage of the file it is given and cannot pass --target, so this file exists
# only to make the web image last. Point the web service at it with RAILWAY_DOCKERFILE_PATH=web.Dockerfile.
#
# Everything below is identical to Dockerfile with the TARGET selector removed. Do not edit this file
# by hand — edit Dockerfile; backend/test/dockerfiles.test.ts fails if the two drift apart.
